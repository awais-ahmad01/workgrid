...
// import { listFiles, requestUpload, deleteFileApi } from '@/lib/files'
// import { supabaseBrowser } from '@/lib/supabaseClient'
// import { ArrowLeft, Send, Paperclip, Trash2, Download, Loader2 } from 'lucide-react'
...
//     const data = await apiFetch(`${API_BASE}/tasks/${taskId}`, { token })
//     setTask(data.task)
//   }
...
//       setLoading(true)
//       await fetchTask()
//       const commentsResp = await fetchComments({ token, taskId })
//       setComments(commentsResp.data.comments || [])
//       const filesResp = await listFiles({ token, taskId })
...
//         // Refetch comments to get user names
//         fetchComments({ token, taskId }).then(resp => setComments(resp.data.comments || [])).catch(() => {})
//       })
//       .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'task_files', filter: `task_id=eq.${taskId}` }, () => {
//         listFiles({ token, taskId }).then(resp => setFiles(resp.data.files || []))
//       })
...
//     try {
//       const resp = await postComment({ token, taskId, body: commentInput })
//       setCommentInput('')
//       // Add comment optimistically, but realtime will also fire
//       // Deduplication in realtime handler will prevent duplicates
//       setComments((prev) => {
//         // Check if already exists (from realtime that fired first)
//         if (prev.some(c => c.id === resp.data.comment.id)) {
//           return prev
//         }
//         return [...prev, resp.data.comment]
//       })
//     } finally {
...
//     setUploading(true)
//     try {
//       const resp = await requestUpload({ token, taskId, file })
//       if (resp?.data?.file) {
//         setFiles((prev) => [resp.data.file, ...prev])
//       } else {
//         // refresh
//         const filesResp = await listFiles({ token, taskId })
//         setFiles(filesResp.data.files || [])
...
//     try {
//       await deleteFileApi({ token, taskId, fileId })
...
//           {task.description && (
//             <p className="text-gray-600 mt-2 max-w-3xl whitespace-pre-wrap">{task.description}</p>
//           )}
...
//                 <div className="flex items-center gap-2">
//                   <a
//                     href={`https://ahxjpxkhkebnjreojakc.supabase.co/storage/v1/object/public/task-files/${f.file_path}`}
//                     target="_blank"
//                     rel="noreferrer"
